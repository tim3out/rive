...
async def send_traceback(
    ...
    for page in paginator.pages:
        embed = discord.Embed(description=f"{page}", color=0x2b2d31).set_footer(text="rive", icon_url="https://cdn.discordapp.com/attachments/1208741351760461846/1234890090283667567/1203853126486990909.gif?ex=66326084&is=66310f04&hm=3c0bb03f86cb839aded3f573cf6ad5cd83de0fd5fefdae3ca62d7bcfaf8085d6&")

    if isinstance(destination, discord.Message):
        message = await destination.reply(embed=embed)    
    else:
...
class ReplResponseReactor:
    ...
    async def __aenter__(self):
        self.handle = self.loop.create_task(
            do_after_sleep(
                ...
                self.message, '<:next:1246886309721669703>'
            )
        )
        return self
    ...
    async def __aexit__(
        ...
        if not exc_val:
            await attempt_add_reaction(self.message, '<:check:1232712864352047144>')
            return False
        ...
        if isinstance(exc_val, (SyntaxError, asyncio.TimeoutError, subprocess.TimeoutExpired)):
            ...
            if destination != self.message.channel:
                await attempt_add_reaction(
                    self.message,
                    '<:cross:1232721067995435059>' if isinstance(exc_val, SyntaxError) else '<:cross:1232721067995435059>'
                )
            ...
            if destination != self.message.channel:
                await attempt_add_reaction(self.message, '<:cross:1232721067995435059>')
